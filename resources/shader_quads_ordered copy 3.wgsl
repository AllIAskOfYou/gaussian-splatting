...
fn vs_main(@builtin(instance_index) instanceIndex: u32, vertex: VertexInput) -> VertexOutput {
	...
	splat_view = splat_view * transpose(splat_view);
	...
	var local_pos = vec4f(0.3*pos2d, 0, 0.0);
	//var local_pos = vec4f(0.2*vertex.position, 0, 0.0);
	//pos.x *= variance[0][0];
	//pos.y *= variance[1][1];
	//pos.z *= variance[2][2];
	//var local_pos = vec4f(splat_xyz * vertex.position, 0.0);
	//var local_pos = splat_view * vec4f(0.1*vertex.position, 0.0, 1.0);
	var vertex_pos = local_pos + (uniforms.projectionMatrix * splat_center);
	vertex_pos.w = 10.0f;
	//center = uniforms.projectionMatrix * center;
	//pos = uniforms.projectionMatrix * pos;
	//pos.w = 0.0;
